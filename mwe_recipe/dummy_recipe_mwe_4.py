...
@run.cli.factory(name="dummy_recipe")  # Using run.cli.factory
def dummy_recipe(
    max_steps: int = 10,
    dir: str = "./dummy_checkpoints",
    name: str = "dummy_experiment",  # name argument is part of the recipe
    num_nodes: int = 1,  # num_nodes is typically for distributed setup
    ...
    # Return a run.Partial wrapping the training function with instantiated objects
    return run.Partial(top_level_train_fn, model=model, data_module=data_module, trainer_config=trainer_config)
...
def main():
    ...
    recipe_partial = dummy_recipe()  # This returns a Partial of top_level_train_fn
